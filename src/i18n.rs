    ...
    pub fn get(lang: Language, key: &str) -> String {
        match (lang, key) {
            ...
            (Language::En, "cmd_help") => format!(
                ...
                "[Benchmark]".yellow().bold(),
                "bench quick".white().bold(), "Quick perf benchmark",
                "bench paper".white().bold(), "Full ACHF paper suite",
                "bench <exp>".white().bold(), "Run single experiment",
                "bench list".white().bold(), "List experiments",
                "[Info]".yellow().bold(),
            ...
            (Language::Cn, "cmd_help") => format!(
                ...
                "[性能测试]".yellow().bold(),
                "bench quick".white().bold(), "快速性能基准",
                "bench paper".white().bold(), "ACHF 论文级全套实验",
                "bench <exp>".white().bold(), "运行单项实验",
                "bench list".white().bold(), "查看实验列表",
                "[信息]".yellow().bold(),
            ...
            // === Bench Commands (CLI) ===
            (Language::En, "bench_quick_start") => "[Bench] Running quick benchmark...".to_string(),
            (Language::Cn, "bench_quick_start") => "[Bench] 运行快速基准测试...".to_string(),
            (Language::En, "bench_list_header") => "\n[Bench] Available experiments:".to_string(),
            (Language::Cn, "bench_list_header") => "\n[Bench] 可用实验:".to_string(),
            (Language::En, "bench_list_ablation") => "ablation     — dense, static-pruning, and selector controls".to_string(),
            (Language::Cn, "bench_list_ablation") => "ablation     — 稠密、静态剪枝与选择器对照".to_string(),
            (Language::En, "bench_list_mode") => "mode         — fixed paths, plain EMA, and guarded AMA".to_string(),
            (Language::Cn, "bench_list_mode") => "mode         — 固定路径、朴素 EMA 与完整 AMA 对比".to_string(),
            (Language::En, "bench_list_path") => "path         — Cached/Sparse/Dense inference path latency".to_string(),
            (Language::Cn, "bench_list_path") => "path         — Cached/Sparse/Dense 推理路径延迟".to_string(),
            (Language::En, "bench_list_gate") => "gate         — Gate dynamics during training".to_string(),
            (Language::Cn, "bench_list_gate") => "gate         — 训练过程中门控动态曲线".to_string(),
            (Language::En, "bench_list_scale") => "scale        — Throughput at different rank".to_string(),
            (Language::Cn, "bench_list_scale") => "scale        — 不同 rank 下的吞吐量".to_string(),
            (Language::En, "bench_list_apply") => "apply        — FFN/Attention/DQN combination".to_string(),
            (Language::Cn, "bench_list_apply") => "apply        — FFN/Attention/DQN 组合效果".to_string(),
            (Language::En, "bench_list_convergence") => "convergence  — loss + reward convergence curves".to_string(),
            (Language::Cn, "bench_list_convergence") => "convergence  — loss + reward 收敛曲线".to_string(),
            (Language::En, "bench_usage_header") => "\nUsage:".to_string(),
            (Language::Cn, "bench_usage_header") => "\n用法:".to_string(),
            (Language::En, "bench_usage_quick") => "  bench quick              Quick benchmark".to_string(),
            (Language::Cn, "bench_usage_quick") => "  bench quick              快速基准测试".to_string(),
            (Language::En, "bench_usage_paper") => "  bench paper              Run all 7 experiments (default 5 trials)".to_string(),
            (Language::Cn, "bench_usage_paper") => "  bench paper              运行全部 7 项实验（默认 5 trials）".to_string(),
            (Language::En, "bench_usage_only") => "  bench paper --only ablation,gate".to_string(),
            (Language::Cn, "bench_usage_only") => "  bench paper --only ablation,gate".to_string(),
            (Language::En, "bench_usage_trials") => "  bench paper --trials 5 --format png".to_string(),
            (Language::Cn, "bench_usage_trials") => "  bench paper --trials 5 --format png".to_string(),
            (Language::En, "bench_usage_output") => "  bench paper --output-dir results".to_string(),
            (Language::Cn, "bench_usage_output") => "  bench paper --output-dir results".to_string(),

            // === F2P Result Lines ===
            (Language::En, "f2p_prob_line") => "Probability to get UP with ONLY free resources: {:.2}%".to_string(),
            (Language::Cn, "f2p_prob_line") => "仅用免费资源抽到UP的概率: {:.2}%".to_string(),
            (Language::En, "f2p_prob_line_max") => "Probability to get UP with ONLY free resources: ≥99.99% (all succeeded)".to_string(),
            (Language::Cn, "f2p_prob_line_max") => "仅用免费资源抽到UP的概率: ≥99.99% (全部成功)".to_string(),
            (Language::En, "f2p_cost_line") => "Avg Extra Jade Cost: {:.0} (Approx. {:.1} extra pulls)".to_string(),
            ...
            (Language::Cn, "sys_demo_mmap") => "[系统] 演示高性能张量 I/O (Mmap)...".to_string(),

            // === Input Validation ===
            (Language::En, "input_capped") => "Input {} exceeds limit, capped to {}.".to_string(),
            (Language::Cn, "input_capped") => "输入 {} 超出限制，已自动调整为 {}。".to_string(),
            (Language::En, "prompt_yn_retry") => "Please enter y or n: ".to_string(),
            (Language::Cn, "prompt_yn_retry") => "请输入 y 或 n: ".to_string(),

            // === Command Suggestion ===
            (Language::En, "cmd_suggest") => "Unknown command \"{}\". Did you mean \"{}\"?".to_string(),
            (Language::Cn, "cmd_suggest") => "未知命令 \"{}\"，你是不是想输入 \"{}\"？".to_string(),

            // === Pool List Table Headers ===
            (Language::En, "tbl_pool_id") => "#".to_string(),
            (Language::Cn, "tbl_pool_id") => "#".to_string(),
            (Language::En, "tbl_pool_name") => "Name".to_string(),
            (Language::Cn, "tbl_pool_name") => "名称".to_string(),
            (Language::En, "tbl_pool_type") => "Type".to_string(),
            (Language::Cn, "tbl_pool_type") => "类型".to_string(),
            (Language::En, "tbl_pool_up") => "UP".to_string(),
            (Language::Cn, "tbl_pool_up") => "UP".to_string(),
            (Language::En, "tbl_pool_status") => "Status".to_string(),
            (Language::Cn, "tbl_pool_status") => "状态".to_string(),
            (Language::En, "pool_status_active") => "Active".to_string(),
            (Language::Cn, "pool_status_active") => "当期".to_string(),
            (Language::En, "pool_status_archived") => "Archived".to_string(),
            (Language::Cn, "pool_status_archived") => "往期".to_string(),

            // === History Table Headers ===
            (Language::En, "tbl_hist_id") => "#".to_string(),
            (Language::Cn, "tbl_hist_id") => "#".to_string(),
            (Language::En, "tbl_hist_pool") => "Pool".to_string(),
            (Language::Cn, "tbl_hist_pool") => "卡池".to_string(),
            (Language::En, "tbl_hist_pulls") => "Pulls".to_string(),
            (Language::Cn, "tbl_hist_pulls") => "抽数".to_string(),
            (Language::En, "tbl_hist_sims") => "Sims".to_string(),
            (Language::Cn, "tbl_hist_sims") => "模拟次数".to_string(),
            (Language::En, "tbl_hist_avg6") => "Avg 6*".to_string(),
            (Language::Cn, "tbl_hist_avg6") => "均6星".to_string(),
            (Language::En, "tbl_hist_avgup") => "Avg UP".to_string(),
            (Language::Cn, "tbl_hist_avgup") => "均UP".to_string(),
            (Language::En, "tbl_hist_time") => "Time".to_string(),
            (Language::Cn, "tbl_hist_time") => "耗时".to_string(),
