    ...
    pub fn get(lang: Language, key: &str) -> String {
        match (lang, key) {
            ...
            (Language::En, "header_rules") => {
                "Rules: {} Pulls Guarantee 6* (50/50 UP, No Guarantee on Loss)".to_string()
            }
            (Language::Cn, "header_rules") => {
                "规则: {} 抽必出6星 (50/50 UP, 歪了不保底)".to_string()
            }

            (Language::En, "header_big_pity") => {
                "Big Pity: Cumulative {} pulls guarantee UP (Once per pool)".to_string()
            }
            ...
            (Language::En, "prompt_welfare") => format!(
                "{}",
                "Use Welfare Resources ({} pulls)? (y/n, default y): ".yellow()
            ),
            (Language::Cn, "prompt_welfare") => {
                format!("{}", "使用福利资源 ({} 抽)? (y/n, 默认 y): ".yellow())
            }

            (Language::En, "prompt_sim_count") => format!(
                "{}",
                "Enter simulation count (default 1, max 1M): ".yellow()
            ),
            (Language::Cn, "prompt_sim_count") => {
                format!("{}", "输入模拟次数 (默认 1, 最大 1M): ".yellow())
            }
